...
This module provides grid search for Double Q-learning and A2C-GAE, evaluating
configurations across multiple random seeds to find robust
...
def tune_doubleq(args) -> None:
    ...
    hyperparameter_grid = {
        ...
        "gamma": [0.95, 0.99, 1.0],
        "eps_end": [0.01, 0.05],
        "eps_decay_episodes": [50_000, 100_000, 200_000, 300_000],
    }
...
def tune_a2c(args) -> None:
    """Run grid search hyperparameter tuning for A2C-GAE.
    ...
    for trial_num, hyperparameter_values in enumerate(
        tqdm(combinations, desc="grid A2C"), start=1
    ):
        config_kwargs = dict(zip(grid_keys, hyperparameter_values))
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Optional cap on grid combinations (0 = all).",
    )
